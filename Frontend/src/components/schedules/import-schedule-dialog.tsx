...
export function ImportScheduleDialog({ open, onClose, onSuccess }: ImportScheduleDialogProps) {
  ...
  const handleImport = async () => {
    ...
    if (!Array.isArray(schedules)) {
      setError('Dữ liệu phải là một mảng các lịch làm việc.');
      return;
    }
    ...
    try {
      const response = await fetch('http://localhost:5000/api/import-schedules/work-schedules', {
        method: 'POST',
        headers: {
    ...
    >
      <DialogTitle>
        <Stack direction="row" spacing={1} alignItems="center">
          <Upload size={24} />
          <Typography variant="h6">Import Lịch Làm Việc từ JSON</Typography>
        </Stack>
      </DialogTitle>

...
import { TeacherSchedule, TeacherTimeSlot, TeacherScheduleDay } from '@/types';
...
// Định nghĩa interface cho dạng xem tất cả giáo viên
interface AllTeachersScheduleDay {
  date: string;
  weekday: string;
...
export function TeacherScheduleList({ scheduleData }: TeacherScheduleListProps) {
  ...
  const teacherData = useMemo(() => {
    ...
    if (viewAllTeachers) {
      ...
      const dateMap: Record<string, AllTeachersScheduleDay> = {};
  ...
  const stats = useMemo(() => {
    ...
    if (viewAllTeachers) {
      ...
      (teacherData as TeacherScheduleDay[]).forEach(day => {
        byDay[day.date] = { total: 0, teaching: 0, available: 0 };
  ...
  }, [teacherData, viewAllTeachers, teacherList]);
  ...
  const renderListView = () => {
    ...
    return (
      <Stack spacing={3}>
        {viewAllTeachers 
          ? // Hiển thị cho tất cả giáo viên
            (teacherData as AllTeachersScheduleDay[]).map((day) => {
              // Kiểm tra xem ngày này có phải là hôm nay không
              const todayCheck = isToday(day.date);
              
              // Tính tổng số buổi dạy và khung trống của mỗi giáo viên
              let teachingCount = 0;
          ...
          : // Hiển thị cho một giáo viên
            (teacherData as TeacherScheduleDay[]).map((day) => {
              // Kiểm tra xem ngày này có phải là hôm nay không
              const todayCheck = isToday(day.date);
              
              // Tính số slot theo trạng thái
                        ...
                        >
                          {teachingSlots.length > 0 && (
                            <Chip 
                              size="small" 
                              color="error"
                              label={`${teachingSlots.length} buổi dạy`} 
                              sx={{ mb: { xs: 0.5, md: 0 } }}
                            />
                          )}
                          {availableSlots.length > 0 && (
                            <Chip 
                              size="small" 
                              color="success"
                              label={`${availableSlots.length} khung trống`} 
                              sx={{ mb: { xs: 0.5, md: 0 } }}
                            />
                          )}
                          <Chip 
